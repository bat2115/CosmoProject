...
omegaM_array = arange(0.22, 0.325, 0.0005) #dx = 0.0005
w_array = arange(-1.2, -0.95, 0.0005) #dx = 0.0008

def Likelihood(omega_M, w_, script_M):
    chisqd = sum( ( (mag_data - mag(omega_M, w_, script_M)) / error )**2.0 )
    integrand = lambda script_M: exp(-chisqd / 2.0)
    L = integrate.quad(integrand, script_M-3.0, script_M+3.0)[0]
    return L

scriptL = zeros( (len(omegaM_array), len(w_array)) )
print shape(scriptL), shape(omegaM_array), shape(w_array)
for i in range(len(omegaM_array)):
    for j in range(len(w_array)):
        scriptL[i,j] = Likelihood(omegaM_array[i], w_array[j], scriptM)

print omegaM_array[where(scriptL == amax(scriptL))[0]], w_array[where(scriptL == amax(scriptL))[1]]


plt.figure(0)
x, y = meshgrid(w_array, omegaM_array)
